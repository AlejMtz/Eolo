...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnEditar').addEventListener('click', function() {
        window.location.href = `entrega_turno.html?id=${entregaId}`;
    });
...
async function cargarDetalleEntrega() {
    try {
        const response = await fetch(`entrega_turno_leer_id.php?id=${entregaId}`);
...
async function eliminarEntrega(id) {
    try {
        ...
        const response = await fetch('entrega_turno_eliminar.php', {
            method: 'POST',
            body: formData
        });
